...
function StatCard({
  label, value, icon: Icon, variant = 'default', linkTo, sublabel,
}: {
  label: string; value: string | number; icon: React.ElementType
  variant?: 'default' | 'warning' | 'danger'; linkTo?: string; sublabel?: string
}) {
  const colorClass = variant === 'danger' ? 'text-destructive' : variant === 'warning' ? 'text-amber-600' : 'text-foreground'
  const iconColorClass = variant === 'danger' ? 'text-destructive' : variant === 'warning' ? 'text-amber-500' : 'text-muted-foreground'
...
function MetricBar({ label, value, max, unit, colorClass = 'bg-primary' }: {
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex justify-between text-xs text-muted-foreground">
        <span>{label}</span>
        <span className="font-medium text-foreground">{value} <span className="font-normal opacity-60">{unit}</span></span>
      </div>
...
function DashboardPage() {
  const { data: dash, isLoading } = useQuery({
    ...
    // No userId in the URL — backend reads the user from JWT
  ...
  const subjectsMap = React.useMemo(
    () => (subjectsList || []).reduce((acc: Record<number, string>, s: any) => { acc[s.id] = s.subjectName; return acc }, {}),
    [subjectsList],
  )

  const processedLectures = dash?.completedLectures ?? 0
  const pendingRevisions  = dash?.pendingRevisionsCount ?? 0
  const upcomingDeadlines = dash?.upcomingDeadlines?.length ?? 0
  const studyMinutesWeek  = dash?.studyHoursThisWeek ?? 0
      ...
      {isLoading ? (
        ...
        <>
          <div className="grid gap-4 md:grid-cols-3">
            <StatCard label="Lectures processed" value={processedLectures} icon={BookOpen} linkTo="/lectures" sublabel="Click to view all lectures" />
            <StatCard
              label="Pending revisions" value={pendingRevisions} icon={Clock}
              variant={pendingRevisions > 10 ? 'warning' : 'default'}
              linkTo="/revisions"
              sublabel={pendingRevisions > 0 ? 'Click to start revising' : 'All caught up!'}
            />
            <StatCard
              label="Upcoming deadlines" value={upcomingDeadlines} icon={AlertCircle}
          ...
          <div className="grid gap-4 md:grid-cols-2">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex justify-between text-xs text-muted-foreground pt-1">
                  <span className="flex items-center gap-1">
                    <CheckCircle2 className="h-3 w-3 text-emerald-500" />{completedSessions} sessions done
                  </span>
                  <span className="flex items-center gap-1">
                    <CalendarCheck className="h-3 w-3" />{totalSessions} total planned
                  </span>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <MetricBar label="Study time" value={studyMinutesWeek} max={metricsMax} unit="min" colorClass="bg-primary" />
                <MetricBar label="Sessions completed" value={completedSessions} max={metricsMax} unit="sessions" colorClass="bg-emerald-500" />
                <MetricBar label="Total sessions planned" value={totalSessions} max={metricsMax} unit="sessions" colorClass="bg-muted-foreground/40" />
              </CardContent>
            </Card>
          </div>
              ...
              <CardContent>
                <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                  {dash.todayStudyPlan.map((plan: any) => (
                    <div key={plan.id} className="flex items-center justify-between p-3 rounded-lg border bg-muted/30 text-sm">

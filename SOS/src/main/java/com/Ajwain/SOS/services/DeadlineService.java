...
public class DeadlineService {
	...
	public PaginationResponseDTO<DeadlineResponseDTO> getDeadlinesByUser(long userId,Pageable pageable){
		pageable=validatePageable(pageable);
			
	
		Page<Deadline> deadline=deadlineRepository.findBySubjectUserId(userId,pageable);
		List<DeadlineResponseDTO> dtos=deadline.getContent().stream().map(this::convertToResponseDTO).toList();
		return PaginationResponseDTO.fromPage(deadline,dtos);
	}
	...
	public PaginationResponseDTO<DeadlineResponseDTO> getDeadlines(DeadlineSearchCriteria criteria,
	        Pageable pageable){
		pageable=validatePageable(pageable);
		Specification<Deadline> s=buildSpecification(criteria);
